...
void recursive_dictionary_destroy(struct Node *cursor)
{
    // CRITICAL: Check if cursor is NULL first!
    if (cursor == NULL) {
        return;
    }
    ...
    if (cursor->data) {
        ...
        // DON'T call free(cursor->data) - entry_destructor already does it!
        cursor->data = NULL;  // Set to NULL to prevent accidental reuse
    }
